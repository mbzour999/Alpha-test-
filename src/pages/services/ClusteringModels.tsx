import { useTranslation } from 'react-i18next';
import Navigation from '@/components/Navigation';
import Footer from '@/components/Footer';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Layers, Users, Target, BarChart, ArrowRight, CheckCircle } from 'lucide-react';
import { Link } from 'react-router-dom';

const ClusteringModels = () => {
  const { t } = useTranslation();

  const features = [
    {
      icon: Layers,
      title: 'Advanced Clustering',
      description: 'K-means, hierarchical, and density-based clustering algorithms for diverse data types.'
    },
    {
      icon: Users,
      title: 'Customer Segmentation',
      description: 'Identify distinct customer groups based on behavior, preferences, and demographics.'
    },
    {
      icon: Target,
      title: 'Market Segmentation',
      description: 'Discover market opportunities and optimize targeting strategies with data-driven segments.'
    },
    {
      icon: BarChart,
      title: 'Pattern Recognition',
      description: 'Uncover hidden patterns and relationships in complex datasets automatically.'
    }
  ];

  const applications = [
    'Customer segmentation and persona development',
    'Market research and opportunity identification',
    'Product recommendation groupings',
    'Risk assessment and fraud detection',
    'Inventory categorization and management',
    'Geographic and demographic analysis',
    'Behavioral pattern identification',
    'Anomaly detection and outlier analysis'
  ];

  const useCases = [
    {
      title: 'Customer Segmentation',
      description: 'Group customers based on purchasing behavior, preferences, and lifetime value for targeted marketing.',
      impact: '45% increase in campaign effectiveness'
    },
    {
      title: 'Product Categorization',
      description: 'Automatically categorize products and inventory based on features, sales patterns, and customer preferences.',
      impact: '35% improvement in inventory management'
    },
    {
      title: 'Market Analysis',
      description: 'Identify market segments and opportunities by clustering geographic, demographic, and behavioral data.',
      impact: '50% better market penetration'
    },
    {
      title: 'Fraud Detection',
      description: 'Detect fraudulent transactions by identifying unusual patterns that deviate from normal clusters.',
      impact: '80% reduction in fraud losses'
    }
  ];

  return (
    <div className="min-h-screen">
      <Navigation />
      
      {/* Hero Section */}
      <section className="pt-32 pb-16 bg-gradient-to-br from-alpha-primary/10 to-alpha-secondary/10">
        <div className="alpha-container">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-6xl font-bold text-alpha-foreground mb-6">
              {t('services.clusteringModels.title')}
            </h1>
            <p className="text-xl text-alpha-secondary leading-relaxed mb-8">
              {t('services.clusteringModels.description')}
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              <Button asChild size="lg">
                <Link to="/contact">
                  Discover Patterns
                  <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
              <Button variant="outline" size="lg">
                View Examples
              </Button>
            </div>
          </div>
        </div>
      </section>

      {/* Features Section */}
      <section className="py-16">
        <div className="alpha-container">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-alpha-foreground mb-4">
              Advanced Clustering Techniques
            </h2>
            <p className="text-xl text-alpha-secondary max-w-3xl mx-auto">
              Sophisticated algorithms that reveal hidden patterns and meaningful segments in your data
            </p>
          </div>
          
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature, index) => (
              <Card key={index} className="border-0 shadow-lg text-center">
                <CardContent className="p-6">
                  <div className="w-16 h-16 mx-auto mb-4 bg-alpha-primary/10 rounded-full flex items-center justify-center">
                    <feature.icon className="h-8 w-8 text-alpha-primary" />
                  </div>
                  <h3 className="text-lg font-semibold text-alpha-foreground mb-3">
                    {feature.title}
                  </h3>
                  <p className="text-alpha-secondary leading-relaxed text-sm">
                    {feature.description}
                  </p>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </section>

      {/* Applications Section */}
      <section className="py-16 bg-alpha-muted">
        <div className="alpha-container">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-alpha-foreground mb-6">
                Business Applications
              </h2>
              <p className="text-lg text-alpha-secondary leading-relaxed mb-8">
                Understand customer diversity and market opportunities through intelligent data clustering.
              </p>
              <div className="grid grid-cols-1 gap-3">
                {applications.map((application, index) => (
                  <div key={index} className="flex items-start gap-3">
                    <CheckCircle className="h-5 w-5 text-alpha-primary flex-shrink-0 mt-0.5" />
                    <span className="text-alpha-secondary">{application}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="relative">
              <div className="aspect-square bg-gradient-to-br from-alpha-primary/20 to-alpha-secondary/20 rounded-3xl flex items-center justify-center">
                <Layers className="h-32 w-32 text-alpha-primary" />
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Use Cases Section */}
      <section className="py-16">
        <div className="alpha-container">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-alpha-foreground mb-4">
              Real-World Applications
            </h2>
            <p className="text-xl text-alpha-secondary max-w-3xl mx-auto">
              See how clustering models drive business insights and strategic decisions
            </p>
          </div>
          
          <div className="grid md:grid-cols-2 gap-8">
            {useCases.map((useCase, index) => (
              <Card key={index} className="border-0 shadow-lg">
                <CardContent className="p-8">
                  <h3 className="text-xl font-bold text-alpha-foreground mb-4">
                    {useCase.title}
                  </h3>
                  <p className="text-alpha-secondary leading-relaxed mb-4">
                    {useCase.description}
                  </p>
                  <div className="inline-block px-3 py-1 bg-alpha-primary/10 text-alpha-primary text-sm font-medium rounded-full">
                    {useCase.impact}
                  </div>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-16 bg-gradient-to-r from-[#009ec2] to-[#00e66a] text-white">
        <div className="alpha-container text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">
            Ready to Understand Your Customers?
          </h2>
          <p className="text-xl mb-8 opacity-90 max-w-3xl mx-auto">
            Discover hidden patterns and segments in your data with advanced clustering models.
          </p>
          <Button asChild size="lg" variant="secondary">
            <Link to="/contact">
              Start Customer Segmentation
              <ArrowRight className="ml-2 h-5 w-5" />
            </Link>
          </Button>
        </div>
      </section>

      <Footer />
    </div>
  );
};

export default ClusteringModels;